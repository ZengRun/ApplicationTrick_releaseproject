...
int DES_Encrypt(const char *key, unsigned char *in, unsigned int len, unsigned char *out)
{
	...
	{
		return DES_KEY_LEN_ERROR;
	}

	if(len<=0 || len%8 != 0)
	{
		return DES_IN_DATA_LEN_ERROR;
	}
	...
	for(i=0;i<len/CRYPT_BLOCK_LEN;i++)
	{
		memcpy(plainBlock,in+i*CRYPT_BLOCK_LEN,CRYPT_BLOCK_LEN);
		DES_EncryptBlock(plainBlock,subKeys,cipherBlock);
		memcpy(out+i*CRYPT_BLOCK_LEN,cipherBlock,CRYPT_BLOCK_LEN);
	}
...
int DES_Decrypt(const char *key, unsigned char *in,unsigned int len, unsigned char *out)
{
	...
	for(i=0;i<len/CRYPT_BLOCK_LEN;i++)
	{
		memcpy(cipherBlock,in+i*CRYPT_BLOCK_LEN,CRYPT_BLOCK_LEN);
		DES_DecryptBlock(cipherBlock,subKeys,plainBlock);
		memcpy(out+i*CRYPT_BLOCK_LEN,plainBlock,CRYPT_BLOCK_LEN);
	}
