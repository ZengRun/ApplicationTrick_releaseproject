...
static char *obfuscateKey(char const *src,int length){
	if(src == NULL)
			return NULL;
	int i,number;
	if(length <= 0)
	...
	int mappingArray[] = {5,2,15,7,14,10,8,11,0,9,12,13,1,4,6,3};
...
static int rebuildKey(char *a,int length,char *appKey){
	 int i,number,hnumber,lnumber;
	 int reverseMappingArray[] = {8,12,1,15,13,0,14,3,6,9,5,7,10,11,4,2};
	 for(i = 0; i < length; i++){
			hnumber = *(a + 2 * i);
...
int setIKey_hmac(char *iKey,int n){
	char encKey[44];
	int i;
	base64enc(encKey, (void *)iKey, 32);
	char *a = obfuscateIKey(encKey,44);
	...
	if(a){
		...
		printf("\n");
		if(n==1){
		ret = ompWriteFile(ReinforceHmac1FileName, tmp, 120);
		}
		else if(n==2){
			ret = ompWriteFile(ReinforceHmac2FileName, tmp, 120);
		}
		else if(n==3){
			ret = ompWriteFile(ReinforceHmac3FileName, tmp, 120);
		}
		else{
			return 1;
		}
...
int getwbKeyForGenerate(char *iKey){
	char encodeKey[120] = "";
	int i;
	ompReadFile(ReinforceWBKeyFileName, 120, encodeKey);
...
int gethmacForGenerate(char *iKey,int n){
	char encodeKey[120] = "";
	int i;
	if(n==1){
		ompReadFile(ReinforceHmac1FileName, 120, encodeKey);
	}
	else if(n==2){
		ompReadFile(ReinforceHmac2FileName, 120, encodeKey);
	}
	else if(n==3){
		ompReadFile(ReinforceHmac3FileName, 120, encodeKey);
	}
	else{
		return 1;
	}
