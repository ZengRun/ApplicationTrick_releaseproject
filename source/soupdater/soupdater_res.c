...
//Param 2 is FileName,We need such as HMAC key file and so on later...
	...
	char szHMACRes[LENGTH_HMAC+1] = {0};
	char szHMACOriginRes[LENGTH_HMAC+1] = "4f5479436338457665344e3461744c4757624d44546d714a64684f54346736326d4e452f576953534d36683d";
	...
	char szIKEY_Res[LENGTH_IKEY+1] = {0};
	char szIKEYOrigin_Res[LENGTH_IKEY+1] = "DAoMCA0OBAoAAgKCgsKDAsOCA8LDgwHDwgPBw8LBQcPCAcKDgoMCA0OBgsMCAsLCAILIBAsFCAkIAgcPBwoIAQgHDg4KCAsCCgkLDAgACA0IBwsMCA0HBA==";
//	char szAPPKEY[LENGTH_APPKEY+1] = {0};
//	char szAPPKEYOrigin[LENGTH_APPKEY+1] = "CgIKCg4MDg4KDw4KCggOCwoPBwUIAA4MCgcIDQsODgQKDgoKBwoKBQoKDggOCAoHCgoHAgoPCggKCAgBCA4KCQoLCggLBQgCCgsKCAgACgAKCAgBCgoHBA==";
	...
	//res
	int nread4 = 0;
	FILE *fp4 = fopen("hmac3.reinforce","rb");
	if(fp3 == NULL)
	{
		printf("no hmac3.reinforce");
	}
	else{
	//	return -1;
	nread4 = fread(szHMACRes,sizeof(char),LENGTH_HMAC,fp4);
	if(nread4 == 0 || nread4 < LENGTH_HMAC)
	//	return -1;
	fclose(fp4);
	}
...
//Res
	int nread8 = 0;
	FILE *fp8 = fopen("ikey3.reinforce","rb");
	if(fp8 == NULL)
		{
		printf("no ikey3.reinforce");
	}
	else{
		//return -1;
	nread8 = fread(szIKEY_Res,sizeof(char),LENGTH_IKEY,fp8);
	if(nread8 == 0 || nread8 < LENGTH_IKEY)
		return -1;
	fclose(fp8);
	}
	...
	//RES
	printf("szIKEY_Res is %s\n",szIKEY_Res);
	
	printf("szHMACRes is %s\n",szHMACRes);
...
//res
	printf("replace the ikeyRes\n");
	for (i = 0; i < statbuf.st_size - LENGTH_IKEY; i++ )
	{
		if ( memcmp(str + i , szIKEYOrigin_Res , LENGTH_IKEY) == 0 )
		{
			printf("replace the pawword at : %d.\r\n\r\n",i);
			memcpy( str+i, szIKEY_Res, LENGTH_IKEY);
			//break;
		}
	}

	//Res
	printf("replace the szHMACRes\n");
	for (i = 0; i < statbuf.st_size - LENGTH_HMAC; i++ )
	{
		if ( memcmp(str + i , szHMACOriginRes , LENGTH_HMAC) == 0 )
		{
			printf("replace the szHMACRes at : %d.\r\n\r\n",i);
			memcpy( str+i, szHMACRes, LENGTH_HMAC);
//				break;
		}
	}
